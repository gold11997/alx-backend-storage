...
```sql
Create a SQL script that creates a table 'users' following these requirements:

- The table 'users' should have the following attributes:
  - `id`: An integer that is never null, auto-increment, and the primary key.
  - `email`: A string of 255 characters that is never null and should be unique.
  - `name`: A string of 255 characters.
  - `country`: An enumeration of countries: US, CO, and TN, which is never null (the default will be the first element of the enumeration, here, US).

- If the 'users' table already exists, the script should not fail.

- Your script should be executable on any database system.```
...
A. Procedure AddBonus is taking 3 inputs (in this order):
